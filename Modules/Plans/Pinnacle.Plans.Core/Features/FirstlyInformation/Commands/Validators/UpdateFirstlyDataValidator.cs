...
{
    public class UpdateFirstlyDataValidator : AbstractValidator<UpdateFirstlyDataCommand>
    {
        #region Fields
        ...
        #region Constructors
        public UpdateFirstlyDataValidator(IStringLocalizer<SharedResources> localizer,
        ...
        public void ApplyCustomValidationsRules()
        {
            ...
            RuleFor(x => x.DescriptionAr)
               .MustAsync(async (model, Key, CancellationToken) => !await _firstlyInformationService.IsNameArExistExcludeSelf(Key, model.Id))
               .WithMessage(_localizer[SharedResourcesKeys.IsExist]);
            RuleFor(x => x.DescriptionEn)
              .MustAsync(async (model, Key, CancellationToken) => !await _firstlyInformationService.IsNameEnExistExcludeSelf(Key, model.Id))
              .WithMessage(_localizer[SharedResourcesKeys.IsExist]);
        }
        #endregion
    }
